...
- **Python schema guardrails (Phase 3)**: run `python -m unittest tests.test_phase3_validator` to sanity-check minimal parameter envelopes and holographic alignment locally while package manifests are restored. These tests keep the Signal Bus-facing schemas honest without requiring the full pnpm/Flutter toolchain.
- **Phase 4 specifics**: enable bridge adapters (UDP/OSC/gRPC) with rate limits and error logging; wire replay recorders to capture deterministic fixtures for Unity/Unreal/design-tool overlays. Run `python -m unittest tests.test_phase4_bridge` locally while pnpm/Flutter/Playwright/OpenXR manifests are missing, then rerun `pnpm test:bridges`, `pnpm test:holo`, and `flutter test` once manifests land to validate overlays and holographic clients end to end.
