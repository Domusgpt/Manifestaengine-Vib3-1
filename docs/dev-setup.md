...
- **Phase 2 specifics**: hook React/Vite devtools overlay and Flutter shell to the Signal Bus, then run `pnpm test:render` (Playwright snapshots PNG/WebM), `flutter test` (UI goldens), and `pnpm test:holo` (WebXR/OpenXR HOLO_FRAME replay). Use `just docs:phase2` to verify artifact links and status updates.
