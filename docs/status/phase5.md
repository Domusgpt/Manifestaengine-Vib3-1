...
- [x] Observation CLI can ingest structured logs and exported replays to
      generate consolidated reports for overlays/CLIs while manifests are
      restored.
...
- Added buffered logging and observation reports that combine structured logs,
  health pulses, and replay summaries for downstream overlays and CLIs.
- Created a lightweight observation CLI helper that rebuilds sink health from
  structured logs, hydrates optional replay frames, and emits consolidated
  reports until pnpm/Flutter/OpenXR harnesses return.
