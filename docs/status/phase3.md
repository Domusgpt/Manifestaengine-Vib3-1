...
- Bootstrap tooling for this phase: confirm `pnpm install`, `pnpm exec playwright install --with-deps`, and Flutter/OpenXR harnesses are in place to exercise render/holo tests.
- Implement safety rails and schema validation end-to-end: enforce `agent_frame.v1`, `render_config.v1`, and `holo_intent` envelopes with spawn bounds, focus/handoff validation, and rate limiting before proposals mutate state.
- Wire capability discovery overlays and Signal Bus adapters to surface kernel/backend/schema metadata across web and Flutter shells.
- Land deterministic replay fixtures for agent proposals (wearable + holographic) and hook them into `pnpm test:agents`, `pnpm test:holo`, and `pnpm test:render` alongside `pnpm test` aggregation.
- Update this status doc and architecture/dev docs as validation items complete; capture gaps/blockers and rejection telemetry manifests.
