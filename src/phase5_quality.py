...
def buffered_logger(buffer: list[str] | None = None) -> StructuredLogger:
    """Construct a StructuredLogger that records into an in-memory buffer."""

    lines: list[str] = buffer if buffer is not None else []
    return StructuredLogger(writer=lines.append)
...
def observation_report(
    logs: Sequence[str], monitor: HealthMonitor, recorder: ReplayRecorder | None = None
) -> Mapping[str, Any]:
    """Combine structured logs, health pulses, and replay summaries."""
    ...
    for entry in logs:
        try:
            ...
            parsed_logs.append({"raw": entry, "parse_error": True})
    ...
    if recorder is not None:
        report["replay"] = replay_summary(recorder)
